...
namespace BLL.Services
{
    public class DoctorService : IDoctorService
    {
        ...
        public IEnumerable<TimingViewModel> GetTimingByDoctorId(string doctorId)
        {
            var timingList = _unitOfWork.GenericRepository<Timing>().GetAll()
                .Where(x => x.DoctorId == doctorId).ToList();
            return ConvertModelToViewModelList(timingList);
        }
    }
}
